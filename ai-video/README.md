...
This Wan2.2 client is designed for **Mesulo**, providing video generation capabilities for the Mesulo platform.
...
**Important**: To use LoRA models, upload the LoRA files to the `/loras/` folder in the RunPod Network Volume. The LoRA model names in `lora_pairs` should match the filenames in the `/loras/` folder.
...
This project is based on the following original repositories. All rights to the model and core logic belong to the original authors.
...
The original Wan2.2 project follows its respective license. This Mesulo implementation adheres to that license.
