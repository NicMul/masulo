...
async function generatePromoAsset({ imageUrl, imagePrompt, videoPrompt, userId, accountId, gameId, generateImage, generateVideo }) {
  console.log('🎨 Generating promo asset...');
  ...
  try {
    ...
    // Build prompts (identical to current/theme asset)
    ...
    if (generateImage && !generateVideo) {
      console.log('  🎯 Processing promo asset - generating image and video (image as input for video)');
      ...
      const testImageFilename = `test-promo-${randomString}.jpg`;
      const testVideoFilename = `test-promo-${randomString}.mp4`;
    ...
    // Flow B: Only generateVideo=true
      ...
      console.log('  🎯 Processing promo asset - generating video only');
    ...
    // Flow C: Both generateImage=true and generateVideo=true
      ...
      console.log('  🎯 Processing promo asset - generating both image and video');
    ...
    // Return specific payload format for promo assets
    return {
      assetType: 'promo',
