...
export function KnowledgeGraph() {
  ...
  const resetGraphView = useCallback(() => {
    if (viewMode === '2d') {
      ...
      if (graphRef3D.current) {
        graphRef3D.current.cameraPosition({ x: 0, y: 0, z: 400 }, { x: 0, y: 0, z: 0 }, 0);
      }
    }
  }, [viewMode]);
  ...
  const handleNodeDrag = useCallback((node) => {
    // Pin the node while dragging so the physics engine doesn't "fight" the cursor.
  ...
  const handleNodeDragEnd = useCallback((node) => {
    // Keep the node fixed after release to avoid snapping/jittering back.
